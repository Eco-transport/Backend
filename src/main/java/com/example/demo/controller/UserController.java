...
public class UserController{
    ...
    public User getCompleteUser( ){
        ...
        String username = SecurityContextHolder.getContext( ).getAuthentication( ).getName(); 
        User nuevo = userService.findByUsername(username);
        return new User(nuevo.getId(), nuevo.getUsername());        
    }

    /*@GetMapping( "/getWholeUser" )
    public User getAbsoluteUser( ){
        //Retorna usuario completo, antes solo devolvia nombre e ID, ahora devuelve todo, posibles problemas
        //de seguridad
        ...
        nuevo.getId(), 
        nuevo.getUsername(),
        nuevo.getIdentityNumber(),
        nuevo.getUserPhone(),
        nuevo.getSecurityQuestion(),
        nuevo.getSecurityAnswer(),
        nuevo.getUserState(),
        nuevo.getNames(),
        nuevo.getPassword(),
        nuevo.getSurnames(),
    ...
    public User getAbsoluteUser( ){
        ...
        return new User(
        nuevo.getId(), nuevo.getUsername(),nuevo.getIdentityNumber(),nuevo.getNames(),nuevo.getSurnames()
        );
